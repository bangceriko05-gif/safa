...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.action === 'delete_auth_orphan') {
      ...
      const deletedUserName = (targetUser.user_metadata as any)?.name || targetUser.email || email;
      const deletedUserLabel = `${deletedUserName} <${targetUser.email || email}>`;
      const stamp = new Date().toISOString();
      const auditText = `Akun asal: ${deletedUserLabel}. Akun dihapus permanen pada ${stamp}.`;

      // IMPORTANT:
      // - Beberapa kolom FK ke auth.users TIDAK nullable (contoh: bookings.created_by).
      // - Kalau user dihapus, FK ON DELETE SET NULL akan gagal karena kolom NOT NULL.
      // Solusi: sebelum hapus auth user, kita REASSIGN kolom NOT NULL ke admin yang melakukan aksi,
      // lalu simpan identitas user lama di field catatan/description + activity log.

      // 0) Audit log (biar nama tetap tercatat)
      await supabaseAdmin.from('activity_logs').insert({
        user_id: requestingUserId,
        user_name: 'System',
        user_role: isAdmin ? 'admin' : 'leader',
        action_type: 'user_deleted',
        entity_type: 'AuthOrphan',
        entity_id: targetUser.id,
        description: `Hapus permanen Auth Orphan: ${deletedUserLabel}`,
        store_id: null,
      });

      // 1) Reassign FK yang NOT NULL + simpan auditText di catatan
      // Bookings.created_by (NOT NULL)
      {
        const { data: rows, error } = await supabaseAdmin
          ...
          .limit(1000);
        if (error) throw new Error('Gagal membaca booking untuk proses delete orphan');
        for (const r of rows || []) {
          const nextNote = [r.note, auditText].filter(Boolean).join('\n');
          const { error: upErr } = await supabaseAdmin
            .from('bookings')
            .update({ created_by: requestingUserId, note: nextNote })
            .eq('id', r.id);
          if (upErr) throw new Error('Gagal memindahkan booking.created_by sebelum hapus user');
        }
      }

      // user_permissions.granted_by (FK tanpa cascade)
      {
        const { error } = await supabaseAdmin
          .from('user_permissions')
          .update({ granted_by: requestingUserId })
          .eq('granted_by', targetUser.id);
        if (error) throw new Error('Gagal memindahkan user_permissions.granted_by');
      }

      // user_temp_passwords.created_by (FK tanpa cascade)
      {
        const { error } = await supabaseAdmin
          .from('user_temp_passwords')
          .update({ created_by: requestingUserId })
          .eq('created_by', targetUser.id);
        if (error) throw new Error('Gagal memindahkan user_temp_passwords.created_by');
      }

      // 2) Nullify FK nullable yang bisa menghalangi delete
      {
        const updates = [
          supabaseAdmin.from('bookings').update({ checked_in_by: null }).eq('checked_in_by', targetUser.id),
          supabaseAdmin.from('bookings').update({ checked_out_by: null }).eq('checked_out_by', targetUser.id),
          supabaseAdmin.from('bookings').update({ confirmed_by: null }).eq('confirmed_by', targetUser.id),
          supabaseAdmin.from('booking_requests').update({ processed_by: null }).eq('processed_by', targetUser.id),
          supabaseAdmin.from('room_daily_status').update({ updated_by: null }).eq('updated_by', targetUser.id),
          supabaseAdmin.from('room_deposits').update({ returned_by: null }).eq('returned_by', targetUser.id),
        ];
        for (const p of updates) {
          const { error } = await p;
          if (error) {
            console.log('Warning: gagal nullify salah satu reference (akan lanjut):', error);
          }
        }
      }

      // 3) Hapus data yang memang terkait langsung user (aman karena akan cascade juga)
      ...
      // 4) Storage cleanup helper (tidak wajib selalu ada, tapi aman dicoba)
      {
        const { error } = await supabaseAdmin.rpc('cleanup_storage_for_deleted_user', { p_user_id: targetUser.id });
        if (error) {
          console.log('Note: cleanup_storage_for_deleted_user gagal (akan lanjut):', error);
        }
      }

      console.log(`Cleaned up data for user: ${targetUser.id}`);

      // 5) Delete auth user dengan retry
      ...
      if (lastAuthError) {
        console.error('Error deleting auth user:', lastAuthError);
        throw new Error('Gagal menghapus user dari Auth: ' + lastAuthError.message);
      }
