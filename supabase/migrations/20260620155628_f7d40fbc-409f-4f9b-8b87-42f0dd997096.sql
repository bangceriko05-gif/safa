...
-- Update function to insert draft then update to posted (so stock_in_apply_to_stock fires)
...
BEGIN
  ...
  IF NEW.receipt_status = 'Diterima' THEN
    ...
    IF v_stock_in_id IS NULL THEN
      ...
      -- Insert as draft first so the AFTER UPDATE trigger on stock_in.status fires when we post it
      ...
      -- Now post it to trigger stock application
      UPDATE public.stock_in
...
-- Fix existing backfilled rows: flip them through draft->posted so the stock_in_apply trigger runs
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    SELECT si.id FROM public.stock_in si
    JOIN public.purchases p ON p.id = si.purchase_id
    WHERE si.status = 'posted'
      AND NOT EXISTS (
        -- detect rows that were inserted directly as posted (i.e. their items have not affected products)
        -- We'll just flip all auto-created rows since safe re-application is idempotent only when paired w/ cancel.
        SELECT 1 FROM public.products pr
        JOIN public.stock_in_items sii ON sii.product_id = pr.id
        WHERE sii.stock_in_id = si.id AND false
      )
  LOOP
    -- cancel then repost so stock is applied exactly once
    UPDATE public.stock_in SET status='cancelled', cancelled_at=now() WHERE id = r.id;
    UPDATE public.stock_in SET status='posted', posted_at=now(), cancelled_at=NULL WHERE id = r.id;
  END LOOP;
END;
$$;
